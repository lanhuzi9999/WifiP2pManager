		...
		public void onReceive(Context context, Intent intent) {
			...
			if (GolbalConsts.ACTION_PLAY.equals(action)) {
				...
				Intent target = new Intent(
						GolbalConsts.ACTION_RESPONSE_PLAY_STATE);

				int state = GolbalConsts.STATE_OTHERS;
				if (isPause) {
					state = GolbalConsts.STATE_PAUSE;
					target.putExtra(GolbalConsts.EXTRA_CURRENT_POSITION,
							player.getCurrentPosition());
				} else if (player.isPlaying()) {
					state = GolbalConsts.STATE_PLAYING;
				}
				target.putExtra(GolbalConsts.EXTRA_PLAY_STATE, state);
				sendBroadcast(target);
			} else if (GolbalConsts.ACTION_ACTIVITY_STOPED.equals(action)) {
				// activity停止
				needUpdate = false;
